...
def _handle_media_parse_failure(bot_id, chat_id, result, *, media_type="photo"):
    ...
    if status == "blocked":
        send_message(bot_id, chat_id, "圖片內容被安全阻擋，無法解析。" if media_type == "photo" else "貼圖內容被安全阻擋，無法解析。")
        return
    ...
    if status == "quota_exhausted":
        send_message(
            bot_id,
            chat_id,
            IMAGE_PARSE_QUOTA_EXHAUSTED_TEXT if media_type == "photo" else STICKER_PARSE_QUOTA_EXHAUSTED_TEXT,
        )
        return
    ...
    if status == "service_unavailable":
        send_message(
            bot_id,
            chat_id,
            IMAGE_PARSE_UNAVAILABLE_TEXT if media_type == "photo" else STICKER_PARSE_UNAVAILABLE_TEXT,
        )
        return
    ...
    if status in ["max_tokens_no_response", "no_response"]:
        send_message(
            bot_id,
            chat_id,
            IMAGE_PARSE_LIMIT_EXCEEDED_TEXT if media_type == "photo" else STICKER_PARSE_LIMIT_EXCEEDED_TEXT,
        )
        return
    ...
    send_message(
        bot_id,
        chat_id,
        "圖片解析失敗，文字回應一併停止，請稍後再試。" if media_type == "photo" else "貼圖解析失敗，文字回應一併停止，請稍後再試。",
    )
...
""".strip()
    ...
    parse_result = ask_gemini_image_to_text(
    ...
    if not parse_result.get("ok") or not description:
        _handle_media_parse_failure(bot_id, chat_id, parse_result, media_type="photo")
        return
    ...
    synthetic_text = (
        ...
        f"{_text(parse_result.get('model')) or 'Gemini 圖片模型'} 圖片解析：\n"
        f"{description}\n\n"
...
""".strip()
    ...
    if not parse_result.get("ok") or not description:
        _handle_media_parse_failure(bot_id, chat_id, parse_result, media_type="sticker")
        return
