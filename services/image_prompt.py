...
IMAGE_TASK_PROMPT = """
先整理訊息內容：把以上文字轉成一張圖片可以呈現的明確畫面，只保留人物外觀、服裝、姿勢、動作、表情、場景、光線與構圖。若文字含有對話、括號動作或敘事，請理解其畫面含義後自然呈現，不要把文字、對話框、字幕或浮水印畫進圖片。以參考圖中的人物臉部與主要特徵作為同一人物。遮罩以外觀身份為錨點，請依本輪訊息與固定標籤重新生成身體姿勢、動作、服裝、場景、光線與構圖；不要沿用基準圖的白色背景、站姿或原服裝。保持寫實照片風格、自然人體比例、清楚五官與完整構圖。只生成圖片，不要輸出任何文字說明。
""".strip()
